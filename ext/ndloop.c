...
ndloop_alloc(ndfunc_t *nf, VALUE args, volatile void *opt_ptr)
{
    ...
    if (RARRAY_LEN(args) != nf->narg + nf->nopt) {
        ...
        rb_bug("wrong number of arguments for ndfunc (%d for %d)",
               RARRAY_LEN(args), nf->narg + nf->nopt);
    }
