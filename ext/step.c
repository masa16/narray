...
{
    VALUE self = rb_obj_alloc(num_cStep);
...
void
num_step_array_index( VALUE self, size_t ary_size, size_t *plen, ssize_t *pbeg, ssize_t *pstep )
{
    size_t len;
    ssize_t beg, step;
...
void
num_step_sequence( VALUE self, size_t *plen, double *pbeg, double *pstep )
{
    VALUE vbeg, vend, vstep, vlen, a;
    double dbeg, dend, dstep, dsize, err;
    size_t size, n;
...
step_each( VALUE self )
{
    ...
    a = num_step_parameters( self, Qnil );
...
{
    return num_step_new2( range, step, Qnil );
}

static VALUE
range_with_length( VALUE range, VALUE len )
{
    return num_step_new2( range, Qnil, len );
}


static VALUE
num_s_step( int argc, VALUE *argv, VALUE mod )
{
    VALUE self = rb_obj_alloc(num_cStep);
...
VALUE
num_is_sequence( VALUE arg )
{
    if ( rb_obj_is_kind_of(arg, rb_cRange) )
	return Qtrue;
    if ( rb_obj_is_kind_of(arg, num_cStep) )
	return Qtrue;
    return Qfalse;
}
...
Init_num_step()
{
    ...
    num_cStep = rb_define_class_under(mNum, "Step", rb_cObject);
    rb_include_module(num_cStep, rb_mEnumerable);
    rb_define_method(num_cStep, "initialize", step_initialize, -1);

    //rb_define_method(num_cStep, "each", step_each, 0);

    rb_define_method(num_cStep, "first", step_first, 0);
    rb_define_method(num_cStep, "last", step_last, 0);
    rb_define_method(num_cStep, "begin", step_first, 0);
    rb_define_method(num_cStep, "end", step_last, 0);
    rb_define_method(num_cStep, "step", step_step, 0);
    rb_define_method(num_cStep, "length", step_length, 0);
    rb_define_method(num_cStep, "size", step_length, 0);
    rb_define_method(num_cStep, "exclude_end?", step_exclude_end_p, 0);
    //rb_define_method(num_cStep, "to_s", step_to_s, 0);
    //rb_define_method(num_cStep, "inspect", step_inspect, 0);
    //rb_define_method(num_cStep, "parameters", num_step_parameters, 1);

    rb_define_method(rb_cRange, "%", range_with_step, 1);
    rb_define_method(rb_cRange, "*", range_with_length, 1);

    rb_define_singleton_method(mNum, "step", num_s_step, -1);
