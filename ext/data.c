...
iter_sort_index(na_loop_t *const lp)
{
    ...
    if (d_idx) {
        ...
        case 8:
            ...
            break;
        default:
            rb_bug("unexpected index type : %d-byte element",i_es);
        }
    } else {
        for (i=0; i<n; i++) {
            // pointer to data
                   ...
                   void (*func_qsort)())
{
    ...
    narray_t *na;
    narray_view_t *nav;
    volatile VALUE idx, self_view, idx_view, info;
    char *buf;
    ndfunc_t *func;
