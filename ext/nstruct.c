...
Init_nstruct()
{
    cNStruct = rb_define_class_under(mNum, "NStruct", cNArray);
    //cNStMember = rb_define_class_under(cNStruct, "Member", rb_cObject);

    //rb_define_alloc_func(cNStMember, nst_member_s_allocate);
    //rb_define_method(cNStMember, "initialize", nst_member_initialize, -1);

    //rb_undef_alloc_func(cNStruct);
    rb_define_singleton_method(cNStruct, "new", nst_s_new, -1);
